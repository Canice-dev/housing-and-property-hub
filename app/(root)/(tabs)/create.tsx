...
export default function CreateScreen() {
  ...
  const handleSubmit = async () => {
    ...
    const subPriceNum = Number(form.subsequent_price);
    if (isNaN(subPriceNum) || priceNum < MIN_PRICE)
      return Alert.alert("Validation", "Price must be greater than ₦0.");
    if (subPriceNum > MAX_PRICE)
      return Alert.alert(
        "Validation",
        `Price cannot exceed ₦${MAX_PRICE.toLocaleString("en-IN")}.`,
      );
    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      >
        ...
        >
          ...
          {/* Initial Price */}
          <View className={sectionClass}>
            <Text className={labelClass}>Initial Price (₦)</Text>
            <TextInput
              className={inputClass}
              placeholder="e.g. 500,000"
              placeholderTextColor="#9CA3AF"
              value={form.initial_price}
              onChangeText={(v) => updateForm({ initial_price: v })}
          ...
          {/* Subsequent Price */}
          <View className={sectionClass}>
            <Text className={labelClass}>Subsequent Price (₦)</Text>
            <TextInput
              className={inputClass}
              placeholder="e.g. 500,000"
              placeholderTextColor="#9CA3AF"
              value={form.subsequent_price}
